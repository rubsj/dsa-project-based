...
def second_highest_stock(stock_counts: List[int], product_ids: List[int]) -> Optional[int]:
    """
...
def aggregate_stock_by_product(product_ids: List[int], stock_counts: List[int]) -> Dict[int, int]:
    """
...
def has_stock_spike_window(stock_counts: List[int], window_size: int, threshold: int) -> bool:
    """
    ...
    """
    slideVal = 0
    for stock in stock_counts[0 , window_size-1]:
        slideVal += stock
...
def longest_mountain_stock_run(stock_counts: List[int]) -> int:
    ...
    isPeak = False
    left = right = 0
    for i in range(1, len(stock_counts)):
        if stock_counts[i] > stock_counts[i - 1]:
            right += 1
